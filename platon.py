...
def imza():
  ...
  ▀▄▀▄▀ █▄█ █▀▄ █▄▀ █▄▄ █ ▄█  █    █ ▀ █ █▀█ █ █ ██▄ █▀▄ 
                                                                 -by mefistophelezz
         """)
    secim = input(" [ ▶ ]  Wordlist Ayrıştırmak İsterseniz [1] yazın yada enter'e basınız... : ")
    if secim == '1':
          try:
            print(color.RED, '\n', '#' * 68,color.BLUE)
...
def sorular():
    ...
    hayvanadi = input(' [+] EVCİL HAYVAN ADI : ')
    KELIMELER.extend(hayvanadi.title().split(','))
    KELIMELER.extend(hayvanadi.split(','))
    ozelisimler = input(' [+] Özel İsimler : ')
...
def secimler():
    ...
    print(color.RED,'#'*68,color.RESET,color.CYAN)
...
def kombinasyon():
    ...
    if secim['sayi'] == 'Y':
        ...
        kombin('[word1 + word2 + str(sayi) for word1 in KELIMELER for word2 in KELIMELER for sayi in range(2000,2020)]')
        ...
        if secim['ozel'] == 'Y':  # eğer hem sayi hemde özel karakterler dahil edildiyse bu bloga girer
            ...
            kombin('[word1 + ozel + word2 + str(sayi) for word1 in KELIMELER for word2 in KELIMELER for ozel in OZEL_KARAKTER for sayi in range(2000,2020)]')
        ...
        if secim['ters'] == 'Y':
            ...
            if secim['ozel'] == 'Y':
                ...
                ## yıl eklenmiş hali
                kombin('[str(sayi) + word1 + ozel + word2 + str(sayi) for word1 in KELIMELER for word2 in KELIMELER for ozel in OZEL_KARAKTER for sayi in range(2000,2020)]')
...
def kaydet():
    ...
    with open(f'{WORDLIST_ISIM}.txt', 'w',encoding='utf-8') as dosya:
        if UZUNLUK == None:
...
# NOT : DAHA BÜYÜK ŞİFRELER İÇİN DAHA FAZLA KARAKTER GİRİN
